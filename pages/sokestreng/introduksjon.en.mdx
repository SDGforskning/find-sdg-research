...
These pages contain search strings used to find academic publications related to certain [Sustainable development goals](https://sdgs.un.org/goals) implemented on this version of baerekraftsforskning.no. 
For the most updated strings see our [GitHub SDG strings repository](https://github.com/SDGforskning/sdg-strings).
...
The approach (action vs. topic) relates to decisions about what kind of research the string should find: research closely related to the actions of the SDG targets (*action*), or research more broadly related to the topics in the targets (*topic*). *You can read more about these below*. Stable versions are saved as "releases"; in between releases, there may be changes to the strings.
...
The strings available at this time (2023) are available for two platforms.
...
* Files with "python" in the file name are python scripts (we provide both an .ipynb notebook, and a copy as a .md markdown file). Here, all covered SDGs (1, 2, 3, 4, 7, 11, 12, 13, 14, 15) are included in one file. This can be run against any list of works in csv format. This script searches with both English and Norwegian terms. 
...
We have finished a version that can be used for ten of the SDGs (in Web of Science search syntax, and in Python), see "Releases". We consider this a "version": We think the strings are functioning relatively well, but they can likely be improved after testing and feedback, and may require changes over time.  
